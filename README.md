...
Below is a real, unedited dashboard generated by running mdpulse over
[github.com/halo-format/halo](https://github.com/halo-format/halo) (50 markdown files).

**Feature Map** — every feature reconstructed from the docs, cross-checked against the current code (badges, doc-vs-code status, code citations, and a verbatim quote per claim):

![mdpulse feature map](docs/preview-feature-map.png)

**Overview** — headline counts, digest, and the auto-built domain glossary:

![mdpulse overview](docs/preview-overview.png)

The full interactive file is in [`examples/halo-dashboard.html`](examples/halo-dashboard.html) — download it and open in any browser (it is self-contained and works offline).
